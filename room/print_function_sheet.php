...
$sql = $db->prepare("SELECT amount FROM tbl_ev_venue_reservations where id='$re_id'");
                        		$sql->execute();
                        		while($row = $sql->fetch()){
    $booking_amount = $row['amount'];
}

//$booking_amount = $venue_id; SELECT * FROM `tbl_ev_venue_reservations` ORDER BY `tbl_ev_venue_reservations`.`amount` ASC
...
<table class="table table-considered">
    ...
    <tbody>
        <tr>
            ...
            <td>1</td>
            <td><?php echo number_format($booking_amount, 2); ?> RWF</td>
            <td><?php echo number_format($booking_amount, 2); ?> RWF</td>
        </tr>
        ...
        $sql->execute();
        $grandTotal = $booking_amount; // Start with venue booking amount
        while($row = $sql->fetch()){
...
<br><br>
<p>Printed By <?php echo $_SESSION['f_name'] . ' ' . $_SESSION['l_name']?></p>
