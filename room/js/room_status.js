$(document).ready(function () {
    const globalApi = '../api/room_status';
    const fetchStatuses = () => {
        $.ajax({
                ...
                if (response.status == 200) {
                    response.room_statuses.forEach(status => {
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn edit-btn" title="Edit" data-id="${status.id}" data-name="${status.status_name}"><i style="font-size:25px;" class="bx bx-edit"></i></button>
                                <button class="btn delete-btn text-danger" title="Delete" data-id="${status.id}"><i style="font-size:25px;" class="bx bx-trash"></i></button>
                            </td>
                        </tr>
                    `;
                    ...
                    statusList.html(`<p>${response.message}</p>`);
                }
            }
        });
    };
    ...
    $('#status-form').on('submit', function (event) {
        ...
        const statusId = $('#status_id').val(); // Call val() as a function
        const statusName = $('#status_name').val();
        const url = statusId ? `${globalApi}/update.php` : `${globalApi}/create.php`;
        const data = statusId ? {
            status_id: statusId,
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (data.status == 201 || data.status == 200) {
                    ...
                    fetchStatuses();
                    $('#status-form')[0].reset(); // Reset the form 
                    $('#status_id').val(''); // Clear the hidden input field
                    $('#status-form button').text('Add Status'); // Reset button text
                } else {
    ...
    $('#status-list').on('click', '.edit-btn', function () {
        ...
        $('#status-form button').text('Update Status');
    });
