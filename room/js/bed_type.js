$(document).ready(function () {
    ...
    const fetchBedTypes = () => {
        $.ajax({
            ...
            success: function (data) {
                ...
                const bedTypeList = $('#bed-type-list');
                bedTypeList.empty();
                ...
                if (response.status == 200) {
                    response.bed_types.forEach(bedType => {
                        ...
                        <tr>
                            <td>${bedType.id}</td>
                            <td>${bedType.bed_type_name}</td>
                            <td>
                                <button class="btn edit-btn" title="Edit" data-id="${bedType.id}" data-name="${bedType.bed_type_name}"><i style="font-size:25px;" class="bx bx-edit"></i></button>
                                <button class="btn delete-btn text-danger" title="Delete" data-id="${bedType.id}"><i style="font-size:25px;" class="bx bx-trash"></i></button>
                            </td>
                        </tr>
                    `;
                        bedTypeList.append(row);
                    });
                } else {
                    console.log(response.message);
                    bedTypeList.html(`<p>${response.message}</p>`);
                }
            }
        });
    };

    // Create and update bed type
    $('#bed-type-form').on('submit', function (event) {
        event.preventDefault();
        const bedTypeId = $('#bed_type_id').val(); // Call val() as a function
        const typeName = $('#type_name').val();
        const url = bedTypeId ? `${globalApi}/update.php` : `${globalApi}/create.php`;
        const data = bedTypeId ? {
            bed_type_id: bedTypeId,
            bed_type_name: typeName
        } : {
            bed_type_name: typeName
        };
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (data.status == 201 || data.status == 200) {
                    ...
                    fetchBedTypes();
                    $('#bed-type-form')[0].reset(); // Reset the form 
                    $('#bed_type_id').val(''); // Clear the hidden input field
                    $('#bed-type-form button').text('Add Bed Type'); // Reset button text
                } else {
    ...
    $('#bed-type-list').on('click', '.edit-btn', function () {
        const id = $(this).data('id');
        const name = $(this).data('name');
        console.log(id, name);
        $('#bed_type_id').val(id);
        $('#type_name').val(name);
        $('#bed-type-form button').text('Update Bed Type');
    });

    // Delete bed type
    $('#bed-type-list').on('click', '.delete-btn', function () {
        const id = $(this).data('id');
