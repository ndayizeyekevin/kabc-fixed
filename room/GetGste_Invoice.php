...
if (isset($_POST['addClientsTocredit'])) {
    ...
    // If no existing client selected, check new fields
    if (empty($client_id) && !empty($_POST['new_fname']) && !empty($_POST['new_phone'])) {
        ...
        $tin = mysqli_real_escape_string($conn, $_POST['new_tin']);
        $email = mysqli_real_escape_string($conn, $_POST['new_email']);
        $created_by = $_SESSION['user_id'] ?? 0;

        // Insert new credit user
        $stmt = $conn->prepare("INSERT INTO creadit_id (f_name, l_name, phone, tinnumber, created_by, created_at, email) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssiss", $fname, $lname, $phone, $tin, $created_by, $time, $email);
        if ($stmt->execute()) {
    ...
    // If we now have a client_id, proceed
    if (!empty($client_id)) {
        // Update order
        $db->prepare("UPDATE tbl_cmd SET creadit_user = ? WHERE OrderCode = ?")->execute([$client_id, $order_code]);

        // Record payment as credit
        $sql = "INSERT INTO payment_tracks (amount, method, order_code, service, created_at, remark)
                VALUES ('$amount', '$method', '$order_code', 'resto', '$time', 'creadit')";
        if ($conn->query($sql)) {
            echo "<script>alert('Credit successfully added');</script>";
        } else {
            echo "<script>alert('Error saving credit');</script>";
        }
    } else {
        echo "<script>alert('No client selected or added');</script>";
    }
}
...
");
	...
	if (isset($_POST['savesales'])) {
		...
		$receipt = '{"custTin": ' . $tin . ',"custMblNo":"' . $phone . '","rptNo":2,"trdeNm":"","adrs":"KN 4 Ave","topMsg":"CENTRE SAINT PAUL LTD\nKN 4 Ave, Kigali, Kigali , Rwanda\nTin: ' . $branch_tin . '\nPhone: ' . $branch_phone . '","btmMsg":"CIS Version 1 Powered by RRA VSDC EBM2.1 \n -------------------------------- \n Welcome","prchrAcptcYn":"N"}';
      ...
      die(var_dump($e));
			...
			"orgSarNo":"' . $invoice . '",
			  ...
			  "itemList":' . $prdct . '
				...
				$receipt_info = addslashes('{"custTin":' . $tin . ',"custMblNo":null,"rptNo":1,"trdeNm":"CSP","adrs":"KN 4 Ave","topMsg":"CENTRE SAINT PAUL LTD\nKN 4 Ave, Kigali, Kigali , Rwanda\nTin: ' . $branch_tin . '\nPhone: ' . $branch_phone . '","btmMsg":"CIS Version 1 Powered by RRA VSDC EBM2.1 \n -------------------------------- \n Welcome","prchrAcptcYn":"N"}');
		...
		");
				...
				$receipt_info = addslashes('{"custTin":"$tin","custMblNo":"0789300811","rptNo":1,"trdeNm":"CSP","adrs":"KN 4 Ave","topMsg":"CENTRE SAINT PAUL LTD\nKN 4 Ave, Kigali, Kigali , Rwanda\nTin: ' . $branch_tin . '\nPhone: ' . $branch_phone . '","btmMsg":"CIS Version 1 Powered by RRA VSDC EBM2.1 \n -------------------------------- \n Welcome","prchrAcptcYn":"N"}');
							...
							<table class="table table-striped" style="font-size:11px;">
								...
								<tfoot>
									<tr>
										...
										<th colspan="1">Total: <?php echo number_format($totprice, 2); $_SESSION['tot_inv_price'] = number_format($totprice, 2); ?></th>
										<th>
											<div class="dropdown">
    ...
    Edit
        ...
        <?php } ?>
						...
						<?php if ($dueamount == 0) { ?>
							...
							<form method="POST">
								<input type="hidden" name="discount_amount" class="form-control" value="<?php echo $discount; ?>">
								<input type="hidden" name="price_amount" class="form-control" value="<?php echo $Mtot; ?>">
								<input type="hidden" name="tot_amount" class="form-control" value="<?php echo $totprice; ?>">
								<input type="hidden" name="waiter" class="form-control" value="<?php echo $waiter; ?>">
								<input type="hidden" name="table" class="form-control" value="<?php echo $table_id; ?>">
								<input type="hidden" name="code" class="form-control" value="<?php echo $_GET['c']; ?>">
								<input type="hidden" name="invoice_no" class="form-control" value="<?php echo $pay_no; ?>">
								<input type="hidden" name="totalitem" class="form-control" value="<?php echo $no; ?>">
								<input type="hidden" name="totalamountamount" value="<?php echo $totalamountamount ?>">
                ...
                <div class="payment-row row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-control" name="method[]" required>
                            ...
                            foreach ($cuntries as $key => $value) {
                                echo '<option value="' . trim($value[1]) . '">' . $value[3] . '</option>';
                            }
                            ?>
                        </select>
                    </div>
      ...
      <form method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="form-group mt-3">
            ...
            <input type="number" min=0 name="creaditamount" class="form-control" required>
          </div>
