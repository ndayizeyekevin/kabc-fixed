...
include '../inc/conn.php';
$names = "";
$sql = "SELECT * FROM tbl_vsdc_sales where pmtTyCd ='02' and transaction_id= '$id'";
$result = $conn->query($sql);
$sale=0;
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
...
$sql = "SELECT sum(amount)  as amount FROM payment_tracks where method = '$method' and order_code= '$code'";
$result = $conn->query($sql);
$sale=0;
if ($result->num_rows > 0) {
  // output data of each row
...
$sql = $db->prepare("SELECT cmd_code  FROM `tbl_cmd_qty` INNER JOIN menu ON menu.menu_id=tbl_cmd_qty.cmd_item WHERE cmd_qty_id > '$from'   AND cmd_status = '12' GROUP BY tbl_cmd_qty.cmd_code");
 }else{
$sql = $db->prepare("SELECT cmd_code FROM `tbl_cmd_qty` INNER JOIN menu ON menu.menu_id=tbl_cmd_qty.cmd_item WHERE cmd_qty_id > '$from' AND cmd_qty_id <= '$last'  AND cmd_status = '12' GROUP BY tbl_cmd_qty.cmd_code");
}
	...
	$sql->execute(array());
                                   ...
                                   $cash = $cash  + getTotalPaidByMethod($fetch['cmd_code'],'01');
                            		   ...
                            		   $creaditdateils = getcreditDetails($fetch['cmd_code']);
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                         <td>Credits: <br>
                                         <?php echo $creaditdateils;?> </td>
                                          <td>   (<?php echo number_format ($credit)?> )<br>
