...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Handle checkout without corporate
    $balance = $_REQUEST['balance'];
    $booking_id = $_REQUEST['booking_id'];
    $room = $_REQUEST['room'];
    $today = date('Y-m-d');
    $sql = "UPDATE  `tbl_acc_booking` SET `booking_status_id`=5, balance='$balance', checked_out_ate='$today' where  id ='$booking_id'";
    if ($conn->query($sql) === TRUE) {
        echo $status = updateRoomStatus($room, 12);
        if ($status == 1) {
            echo "<script>alert('Checked Out successfully');</script>";
            echo "<script>window.location='index?resto=Reservation'</script>";
        }
    }
} 