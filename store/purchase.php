...
if (isset($_GET['delete']) && isset($_GET['req_id'])) {
    ...
    $deleteQuery = "DELETE FROM store_request WHERE req_id = '$req_id'";
    
    if ($conn->query($deleteQuery) === TRUE) {
        echo "<script>
                window.location.href = '?resto=purchases'; // Redirect using JS
              </script>";
        exit();
    }
}
    ...
    <div class="data-table-area">
        <div class="container">
            <div class="row">
				...
				<h3>Manage Purchase <a href="?resto=create_purchase">Create New</a></h3>	
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="data-table-basic" class="table table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                	//	$result = $db->prepare("SELECT * FROM store_request inner join request_store_item on request_store_item.req_id  = store_request.req_id  where status=0 group by  request_store_item.req_id order by store_request.req_id DESC ");
                                	...
                                	$result = $db->prepare("SELECT * FROM store_request  ORDER BY req_id DESC ");
                                		...
                                		while($fetch = $result->fetch()){
                                     	...
                                     	<tr>
                                     	    <td><?php echo $i; ?></td>
                                            <td><a  href="#" data-toggle="collapse" data-target="#demo<?php echo $i?>"><?php echo $fetch['request_from']; ?></a>
                                            <div id="demo<?php echo $i?>" class="collapse">
                                            <table>
                                            <?php 
                                            $result1 = $db->prepare("SELECT * FROM request_store_item WHERE req_id='".$fetch['req_id']."'");
                                        $result1->execute();
                                        $ii=1;
                                        while($fetch1 = $result1->fetch()){
                                            
                          
                                          ?>
                                          <tr>
                                     	      <td><?php echo $ii; ?></td>
                                            <td><?php echo getItemName($fetch1['item_id']); ?></td>
                                             <td><?php echo getItemUnitName(getItemUnitId($fetch1['item_id'])); ?></td>
                                            <td><?php echo $fetch1['qty']; ?> </td>
                                            <td><?php echo number_format(getItemPrice($fetch1['item_id'])); ?> </td>
...
$supplierid = $row['id'];}
	...
	?>			
                                              ...
                                              <a  href="?resto=view_purchase&&id=<?php echo $fetch['req_id']?>&&supplier=<?php echo $supplierid?>">View</a> |
                                              <a href="?resto=purchases&delete=true&req_id=<?php echo $fetch['req_id']; ?>" 
...
?>
              ...
              <script>
              // Live search for purchase table
