...
<?php
	...
	<?php if(isset($_POST['addRequest'])){
		...
		$req_code   = getReqCode($_SESSION['user_id']);
		
		$sql = "INSERT INTO `store_request` (`req_id`, `request_date`, `required_date`, `request_from`, `remark`, `status`, `comment`, `req_code`,supplier) 
		VALUES (NULL, '$requested_date', '$required_date', '$user', '$remark', '0', 'no comment', '$req_code','$supplier');";

if ($conn->query($sql) === TRUE) {
     $last_id = $conn->insert_id;
  echo "<script>window.location='index.php?resto=manageRequest&&id=$last_id' </script>";
} else {
        ...
        <div class="modal-content">
          ...
          <div class="row">
            <div class="form-group">
                <label class="col-md-2 control-label" for=""><strong>Purchaase Date</strong><span class="text-danger">*</span></label>
                <div class="col-md-4">
            ...
            <div class="row">
                    <div class="form-group">
                        <label class="col-md-2 control-label" for=""><strong>Select Supplier</strong></label>
                <div class="col-md-4">
...
?><option value='<?php echo $fetch['id']?>'><?php echo $fetch['name']?></option>
 <?php }
	...
	?>			
            ...
            <input type="submit" name="addRequest" Value="Continue">
            </div>
                    ...
                    <div class="data-table-list " hidden> 
                        ...
                        <div class="table-responsive">
                            <table id="data-table-basic" class="table table-striped" hidden>
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                		$result = $db->prepare("SELECT * FROM  store_request WHERE status!=1  ORDER BY req_id DESC");
                                        $result->execute();
                                		while($fetch = $result->fetch()){
                                		    $i++;
                                     	?>
                                     	<tr>
                                     	    <td><?php echo $i; ?></td>
                                            <td><a  href="?resto=manageRequest&&id=<?php echo $fetch['req_id']?>"><?php echo $fetch['request_from']; ?></a></td>
...
echo $row['name']; }
	...
	?>			
											...
											//	echo 'Approved';
												...
												?><a  href="?resto=manageRequest&&id=<?php echo $fetch['req_id']?>">Manage</a><?php
											}; ?>
