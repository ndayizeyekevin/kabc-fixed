...
$sql = "SELECT * FROM request_store_item  WHERE id='".$_REQUEST['id']."'";
...
if ($result->num_rows > 0) {
  ...
  while($rows = $result->fetch_assoc()) {
                            ...
                            $openning =  getItemPrice($rows['item']) * $rows['last_qty'];
...
<?php
	...
	<?php if(isset($_POST['addRequest'])){
        ...
        $sql_chk = $db->prepare("SELECT * FROM request_store_item WHERE id = '".$_REQUEST['id']."'");
        $sql_chk->execute();
...
$sql = "UPDATE request_store_item SET qty='$qty',del_price='$price' WHERE id='".$_REQUEST['id']."'";
// $sql = "UPDATE request_store_item SET qty='$qty',del_price='$price' WHERE id='".$_REQUEST['id']."'";
if ($conn->query($sql) === TRUE) {
 ...
 echo "<script>history.go(-1);</scrript>";
        ...
        <div class="container">
          ...
          <div class="row">
            <div class="form-group">
              <h4>  <?php echo getItemName($item);?> </h4>
                <hr>
                <label class="col-md-2 control-label" for=""><strong>Purchased QTY <?php echo $rqty?></strong><span class="text-danger">*</span></label>
                <div class="col-md-4">
            <input type="number" name="qty" placeholder="enter Quantity" step=".01" value="<?php echo $rqty ?>" class="form-control">	
	
                </div>
                    <label class="col-md-2 control-label" for=""><strong>Unit Price</strong></label>
                <div class="col-md-4">
                    
                             <input type="number" name="price" placeholder="enter Unit Price" value='<?php echo getItemPrice($item) ?>' class="form-control">	
