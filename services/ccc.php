 ...
 $get_rsv = $db->prepare("SELECT * FROM tbl_tables
   ...
   if(isset($_POST['add']))
   {
     ...
     $sts = 7;
         ...
         $mp = $priceStmt->fetch(PDO::FETCH_ASSOC);
         if (!$mp) { throw new Exception("Menu item not found: " . $menu_id[$i]); }
         $unit_price = isset($mp['menu_price']) ? str_replace(',','',$mp['menu_price']) : 0;

         $sql = $db->prepare("INSERT INTO `tbl_cmd_qty`(`Serv_id`, `unit_price`, `cmd_table_id`,`cmd_item`, `cmd_qty`, `cmd_code`, `cmd_status`, `notify`, `item_name`, `item_code`, `cat_id`, `subcat_id`, `discount`)
         ...
         $sql->execute([':Serv_id' => $Oug_UserID, ':unit_price' => $unit_price, ':cmd_table_id' => $reservID, ':cmd_item' => $menu_id[$i], ':cmd_qty' => $quantity[$i], ':cmd_code' => $ordcode, ':cmd_status' => 13, ':notify' => 0, ':item_name' => $mp['menu_name'] ?? null, ':item_code' => $mp['menu_id'] ?? null, ':cat_id' => $mp['cat_id'] ?? null, ':subcat_id' => $mp['subcat_id'] ?? ($mp['subcat_ID'] ?? null), ':discount' => $mp['discount'] ?? 0]);
...
function fill_product($db){
  ...
  foreach($result as $row){
    $output.='<option value="'.$row['menu_id'].'">'.$row["menu_name"].' ( '.$row["menu_desc"].' ) '.' ( '.$row["menu_price"].' ) </option>';
  }
                    ...
                    <div class="normal-table-list mg-t-30">
                       ...
                       <?php
                        ...
                        $ebmdata="";
                                    ...
                                    WHERE tbl_cmd_qty.cmd_code = '".$_GET['s']."'
                                    GROUP BY cmd_item
                                    ");
                                        $sql_rooms->execute();
                                        $i = 0;
...
<table border="0" align="center" width="100%">
 ...
 <tr>
        <td align="center" style="display: grid;">
           <span>Date'.$bondate .'</span>
        </td>

    </tr>
 <tr>
        <td align="center" style="display: grid;">
           <span>Order Number: '.$ino .'</span>
        </td>
...
<div id="myModalKitechen" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Bon kitchen Preview</h4>
      </div>
      <div class="modal-body" id="printbar">
  <?php   $html = '';
// Sample data from session (modify this with actual session data)
...
<div id="myModalCoffe" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Bon Coffee shop Preview</h4>
      </div>
      <div class="modal-body" id="printbar">
  <?php   $html = '';
// Sample data from session (modify this with actual session data)
...
<table border="0" align="center" width="100%">
 ...
 <tr>
    ...
    <tr>
        <td align="center" style="display: grid;">
           <span> BON DE COMMANDE (Coffee shop) </span>
        </td>
    </tr>
    <tr>
        <td><hr style="border: none; border-top: 1px dashed black; width: 80%;" /></td>
