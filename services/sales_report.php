...
// Final SQL
$sql = "SELECT *,concat(f_name, ' ', l_name) as server, tbl_cmd_qty.created_at, tbl_cmd_qty.cmd_status FROM `tbl_cmd_qty`
                                ...
                                INNER JOIN tbl_users ON tbl_users.user_id = '$user_IId'
$whereClause
                ...
                <tbody>
                    <?php $total = 0; foreach ($orders as $row): ?>
                        <tr>
                            ...
                            <?php
                            // if($status == 3){
                            //             echo '<span class="text-outline text-warning">Pending</span>';
                            //             }elseif($status == 5){
                            //             echo '<span class="text-outline text-info">Confirmed</span>';
                            //             }elseif($status == 12){
                            //             echo '<span class="text-outline text-success">Paid</span>';
                            //             }elseif($status == 8){
                            //             echo '<span class="text-outline text-primary">Delivered</span>';
                            //             }else{
                            //             echo '<span class="text-outline text-danger">Canceled</span>';
                            //             }
                                        ?>
                                        <!--</td>-->
