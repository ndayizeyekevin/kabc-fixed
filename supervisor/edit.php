...
<?php
	...
	<?php if(isset($_POST['addRequest'])){
            ...
            $sqlqty = $db->prepare("UPDATE `tbl_cmd_qty` SET `cmd_qty` = '$item' WHERE `cmd_qty_id` = '".$_REQUEST['id']."'");
            $sqlqty->execute();
        ...
        <div class="container">
          ...
          <div class="row">
            <div class="form-group">
               ...
               <label class="col-md-12 control-label" for=""> Edit command</label><BR><BR>
                   <BR>
                <label class="col-md-2 control-label" for=""><strong>Select Quantity</strong><span class="text-danger">*</span></label>


	
                <div class="col-md-4">
              <?php
			$result = $db->prepare("SELECT * FROM tbl_cmd_qty where cmd_qty_id='".$_REQUEST['id']."'");
                                        $result->execute();
                                		while($fetch = $result->fetch()){
?><input type='number' name="item" max='<?php echo $fetch['cmd_qty']?>' value='<?php echo $fetch['cmd_qty']?>' name="qty">
 <?php }
