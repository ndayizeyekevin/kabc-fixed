        ...
        try{
            ...
            $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    		 ...
    		 for($i=0; $i<count($menu_id); $i++){
    			$sql = "UPDATE `tbl_cmd_qty` SET `cmd_status` = '".$a."' WHERE `cmd_qty_id` = '".$menu_id[$i]."' AND
    			`cmd_code` = '".$orderCode."' ";
    			$acidq = $db->prepare($sql);
    			$acidq->execute();
            ...
            <div class="row">
                ...
                <?php } ?>
                    <div class="normal-table-list mg-t-30">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                 <?php
                                    ...
                                    WHERE tbl_cmd_qty.cmd_code='".$code."' AND tbl_cmd_qty.cmd_status IN(1,13) AND menu.cat_id='2'");
                            		$sql->execute(array());
                            		if($sql->rowCount()){
                                   ...
                                   $cat_id= $fetch['cat_id'];
                            		$subcat_ID = $fetch['subcat_ID'];
                            		 ...
                            		 $bonbar = $bonbar.$fetch['menu_name'].'<br> Qty: '.$fetch['cmd_qty'].'<br> '.$fetch['message'].'<hr style="border: none; border-top: 1px dashed black; width: 100%;" />';
                      ...
                      <br/>
                          <button type="submit" name="proccess" id="proccess" class="btn btn-info btn-sm" style="border-radius: 5px;"><i class="fa fa-step-forward"></i> Complete</button>
                      </form>
                      <hr>
                      <a href="/reciept/bon-bar.php?<?php echo http_build_query($_GET); ?>" 
   class="btn btn-secondary btn-sm" 
   onclick="if(!confirm('Print bon?')) return false; else return true;">
   <i class="fa fa-step-invoice"></i>Print Bon
</a>

                      <!--<a href="https://nstcr.gope.rw/reciept/bon-bar.php?ref=<?php echo $_REQUEST['c']?>" class="btn btn-secondary btn-sm" onclick="if(!confirm('Print bon?'))return false;else return true;"><i class="fa fa-step-invoice"></i>Print Bon</a>-->
                    </div>
                </div>
            </div>
        </div>
