...
<div class="container-fluid py-4">
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center bg-primary text-white">
            <span style="font-size:1.3rem;font-weight:600;">Booking List</span>
            <button type="button" class="btn btn-light btn-sm no-print" onclick="window.print()">
                <i class="fa fa-print"></i> Print
            </button>
        </div>
        <div class="card-body print-section">
            <h4 class="mb-4" style="font-size: 1.5rem;font-weight:bold;">Booking List as on <strong><?php echo date('Y-m-d'); ?></strong></h4>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-striped align-middle mb-0">
                    ...
                    <tbody id="bookingTableBody">
                        <?php 
                        $no = 0;
                        $sql = $db->prepare("SELECT * FROM tbl_acc_booking WHERE booking_status_id IN (1,2,6) ORDER BY created_at DESC");
                        $sql->execute();
                        while($row = $sql->fetch()){
