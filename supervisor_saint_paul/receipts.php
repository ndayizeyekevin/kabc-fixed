    ...
    ?>
                                 <tr class="gradeU">
                    				...
                    				<td><?php echo $fetrooms['custNm']; ?></td>
                                            <td><a href="/reception/org?i=<?=$fetrooms['invcNo']?>&printer=paperoll"><?php echo $fetrooms['invcNo']; ?></a></td>
                                    <td><?php echo $fetrooms['custTin'] != 'null' ? $fetrooms['custTin'] : $fetrooms['custPhone']; ?></td>
...
<?php
    if ($fetrooms['rcptTyCd'] !== 'R') {
        ?> 
        							    <a href="----/reception/refund?ref=<?=$fetrooms['invcNo']?>" class="label label-<?php echo $fetrooms['has_refund'] == 1 ? 'danger' : 'primary' ?>">(<?php echo $fetrooms['has_refund'] == 1 ? 'Refunded' : 'Refund' ?>)</a>
<?php
    }
    ?>
        				            </td>
