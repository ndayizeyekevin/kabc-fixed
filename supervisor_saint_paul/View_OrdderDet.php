                                 ...
                                 <?php
                            	  ...
                            	  $sql = $db->prepare("SELECT * FROM `tbl_cmd_qty`
                                   ...
                                   $serv = $fstsqty['Serv_id'];
                                    ...
                                    <tr>
                                        <?php
                                        // Check if the order is paid (status_id = 12 in tbl_cmd)
                                        $is_paid = ($fstsqty['status_id'] == 12);

                                        if($is_paid){
                                            // Order is paid/checked out - hide checkbox
                                            ?>
...
?>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th><button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#invoiceModal"><i class="fa fa-step-invoice"></i>Invoice</button></th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
www.centrestpaul.com
  ...
  echo $companyInfo;
        ...
        // Get order details
        $sql_order = $db->prepare("SELECT * FROM tbl_cmd WHERE OrderCode = :code LIMIT 1");
        $sql_order->execute([':code' => $_GET['c']]);
        $order_details = $sql_order->fetch();

        $orderNo = isset($order_details['id']) ? $order_details['id'] : '';
        $tableNo = isset($_GET['res']) ? $_GET['res'] : '';
...
function printInvoice() {
  const button = document.getElementById('idinvoice');
  if (button) {
