...
<?php
	...
	<?php if(isset($_POST['addrequest'])){
	     ...
	     $sql = "UPDATE `tbl_cmd_qty` SET `Serv_id` = '$item' WHERE `cmd_code` = '".$_REQUEST['c']."'";
...
if ($conn->query($sql) === TRUE) {
  ...
  echo "Error updating record: " . $conn->error;
	     ...
	     $sql = "UPDATE `tbl_cmd` SET `Serv_id` = '$item' WHERE `OrderCode` = '".$_REQUEST['c']."'";

if ($conn->query($sql) === TRUE) {
  echo "<script>alert('Successfully handed over');</script>";
   echo "<script>window.location='index?resto=OurGste';</script>";
} else {
  echo "Error updating record: " . $conn->error;
}
               ...
               <label class="col-md-12 control-label" for=""> Tranfer Command from <?php echo getIEmployeeName($_REQUEST['res'])?></label><BR><BR>
                ...
                <div class="col-md-4">
               			<select name="item" class="form-control" required>
								<option value=''>Select New waiter</option><?php
