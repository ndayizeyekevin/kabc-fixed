...
if(isset($_POST['add_item'])){
    ...
    // Validate inputs
    if(empty($code) || empty($item) || $qty <= 0) {
        $msge = "Invalid input. Please fill all fields correctly.";
        echo '<meta http-equiv="refresh"'.'content="2;URL=?resto=request&req='.$code.'">'; 
    } else {
        // Check if item already exists in THIS specific request
        $stmt = $db->prepare("SELECT * FROM tbl_request_details WHERE items = ? AND req_code = ?");
        $stmt->execute([$item, $code]);
        $count = $stmt->rowCount();
        
        if($count > 0){
            $msge = "Item is already on this request list!";
            echo '<meta http-equiv="refresh"'.'content="2;URL=?resto=request&req='.$code.'">'; 
        } else {
            // Insert the new item
            $sql = $db->prepare("INSERT INTO tbl_request_details (req_code, items, quantity) VALUES (?, ?, ?)");
            if($sql->execute([$code, $item, $qty])) {
                $msg = "Item added successfully!";
            } else {
                $msge = "Error adding item. Please try again.";
            }
            echo '<meta http-equiv="refresh"'.'content="2;URL=?resto=request&req='.$code.'">'; 
        }
    }
