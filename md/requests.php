...
if(ISSET($_GET['ac'])){
    ...
    $date = date("Y-m-d");
	    ...
	    $sql_upd = $db->prepare("UPDATE tbl_requests SET `status`='3' WHERE `req_code`='".$code."' ");
	    $sql_upd->execute();

    $msg = "Approved Confirmed Successfully";
                            ...
                            <table id="data-table-basic" class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    $i = 0;
                                        ...
                                        INNER JOIN tbl_status ON tbl_requests.status = tbl_status.id WHERE status= 10
                                		...
                                		while($fetch = $result->fetch()){
                                     	...
                                     	<tr>
                                            ...
                                            <td>Veried by Supervisor</td>
                                            <td>
                                                <?php if($fetch['status'] == '10'){ ?>
                                                <a class="btn-sm" href="?resto=request&ac=<?php echo $fetch['req_code'] ?>" onclick="if(!confirm('Do you really want to Confirm This Request?'))return false;else return true;">Approve</a>
                                            <?php } ?>
                                            </td>
                                     	</tr>
         <?php
...
?>
                        ...
                        elseif(!empty($_GET['req'])){
                            ...
                            $_SESSION['preparedby'] =  $getrows['f_name']." ".$getrows['l_name'];
                            $_SESSION['verified_by'] =  getVerifiedby($getrows['verified_by']);
                            $_SESSION['approved_by'] =  getVerifiedby($getrows['approved_by']);
                        ?>
