...
if(isset($_POST['close'])){
 ...
 include '../inc/conn.php';
  ...
  if ($result->num_rows > 0) {
   ...
   $sql = "UPDATE  days SET reviewed_by = '$user' , reviewed_at = '$time'  WHERE id = '$id'";

if ($conn->query($sql) === TRUE) {
  echo "<script>Day Successfull Approved</script>";
} else {
...
echo "<script>alert('Please comfirm data before approving the day')</script>";
}
...
// GROUP BY cmd_item";
                                    ...
                                    INNER JOIN menu ON menu.menu_id=tbl_cmd_qty.cmd_item
                                    WHERE  cmd_qty_id > '$from' AND cmd_qty_id <= '$to' AND cmd_status = '12'
                                    GROUP BY cmd_code");
...
<br>	 <br>
     ...
     <?php include '../holder/printHeader.php'?>
             ...
             <hr>
                        ...
                        <h4><center>Report for <?php echo $reportDate ?></center><h4> <hr>
...
$lastDate = $date->format('Y-m-d');
                            ...
                            ?>
                                  <form method="POST">
                                      <br>
                                      <input type="checkbox" id="approve" name="approve" > As F&B Manager I hereby comfirm  that  I have reviewed and approved this sale report.<br>
                                        <br>
                                    <input type="date"  value="<?php echo $reportDate ?>" class="form-control" name="closedate" required>
                                    <br>
                                <button type="submit" value="Close day" name="close" class="btn btn-info"> Approve </button>
                                </form>
                            <?php
                            endif
                            ?>
