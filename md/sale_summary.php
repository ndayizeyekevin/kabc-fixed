...
$grouped = []; // Associative array to group customer names and sum totals
...
if ($result->num_rows > 0) {
    ...
    // After grouping, create the output string
...
function getClosedByUser($to){
  ...
  $sql = "SELECT * FROM days where date= '$to'";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
...
if ($result->num_rows > 0) {
  ...
  $date = $_POST['closedate'];
   $time = time();
   $from = lastday();
   $sql = "INSERT INTO `days` (`id`, `date`, `from_id`, `to_id`, `created_at`,created_by) VALUES (NULL, '$date', '$from', '$lastid', '$time','$id');";
...
if($last== 0){
  ...
  $sql = $db->prepare("SELECT cmd_code FROM  `tbl_cmd_qty` WHERE cmd_qty_id > '$from' AND  cmd_qty_id <= '$last' AND cmd_status = '12' group by  cmd_code ");
                           ...
                           if($fetch['cmd_code']){
                                 ...
                                 $cashcredit= $cashcredit + getTotalPaidByMethod($fetch['cmd_code'],'03');
                                 $others= $others + getTotalPaidByMethod($fetch['cmd_code'],'07');
                           }
                              }
            ...
            <div class="table-responsive">
                                    ...
                                    <tr>
                                        ...
                                        <tr>
                                         ...
                                         <?php echo getcreditDetails($to);?> </td>
                                          ...
                                          <td>Total:</td>
                                          <td rowspan="2"> <?php echo number_format ($totalresto +  $totallad + $totalcol + $totalcoffe + $totalbar +  $transport - $totalless ) ?> </td>
                                         </tr>
                            ...
                            <table>
                                ...
                                <hr>
                                    ...
                                    <h5>Momo: <?php echo number_format($momo + $momocollection  + $momoadvance -  $momoadvanceless)?> RWF</h5>
                                     ...
                                     <h5>CASH/CREDIT: <?php echo number_format($cashcredit)?> RWF </h5>
                                     <h5>OTHERS: <?php echo number_format($others)?> RWF </h5>

                                        <hr>
                                  <h4> Total:  <?php echo number_format($cash + $card + $momo +  $cheque  + $credit + $cashcredit + $others +
                        ...
                        <br>Signature: .........................................<br>
                            ...
                            <td >
                        <br>Recieved By (G. Cashier):<br><br>Names: .........................................<br>
                        <br>Signature: .........................................<br>
                            </td >
                            </tr>
                            </table>
